...
function extHtml(content, callback, file) {
  var reg = /(<script(?:(?=\s)[\s\S]*?["'\s\w\/\-]>|>))([\s\S]*?)(?=<\/script\s*>|$)|(<style(?:(?=\s)[\s\S]*?["'\s\w\/\-]>|>))([\s\S]*?)(?=<\/style\s*>|$)|<(img|embed|audio|video|link|object|source)\s+[\s\S]*?["'\s\w\/\-](?:>|$)|<!--inline\[([^\]]+)\]-->|<!--(?!\[)([\s\S]*?)(-->|$)|\b(style)\s?=\s?['"][\s\S]*?url\s?\([\s\S]*?\)?['"]/ig;
  callback = callback || function(m, $1, $2, $3, $4, $5, $6, $7, $8, $9) {
    ...
    }else if($9){ // style = "background-image: url()"
      var tag = $9.toLowerCase();
      m = m.replace(/(\s?url\s?)\(('[^']+'\s?\)|"[^"]+"\s?\)|[^>]+\s?)\)/ig, function(_, prefix, value){
        return prefix +'('+ map.uri.wrap(value.replace(/^\s*(.+)\s*$/g, '$1'))+')';
      });
    }
    return m;
  };
